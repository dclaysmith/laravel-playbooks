...
export default {
  ...
  setup(props, { emit }) {
          ...
          for (let [index, item] of newSteps.entries()) {
            ...
            if (oldItem) {
              if (oldItem?.sort_order != item.sort_order) {
                console.log(oldItem.sort_order + " - " + item.sort_order);
                updateStep(item);
              }
            }
          }
    ...
    async function addAction(playbookAction) {
      ...
      playbookActions.value.push(Object.assign(playbookAction, json.data));
    }
    ...
    const loaded = computed(() => {
      return playbookSteps.value !== null && playbookActions.value !== null;
    });
